...
// Deletes a user account using service role after verifying caller is admin
...
serve(async (req) => {
  ...
  if (req.method === "GET") {
    return new Response(JSON.stringify({ status: "ok", message: "admin-delete-user healthy" }), {
      status: 200,
